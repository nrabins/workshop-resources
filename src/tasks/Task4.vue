<template>
  <div>
    <div>
      ...
      <ol class="steps">
        <li>Create a renderless component called FetchData</li>
        <li>Make it accept a URL props where you can pass a url to be fetched.</li>
        <li>Depending on the status: pending, error or resolved (data fetched), make it render different slots to match the use case below.</li>
        <li>
      ...
      <div class="mb-2">
        <WithMousePos>
          <template v-slot="{ x, y }">Mouse position from Renderless component: {{ x }}, {{ y }}</template>
        </WithMousePos>
      </div>
      <div class="mb-2">Mouse position from Composition-API is: {{ mouseX }}, {{ mouseY }}</div>
      <!-- TASK BEGINS HERE -->
      ...
      <template v-if="!error && !isPending">
        <div>
          <AppButton class="mb-3" @click="refresh">Refresh!</AppButton>
          <img :src="data.message" class="result-image">
        </div>
      </template>
    </div>
...
export default {
  ...
  setup() {
    const breed = ref('collie/borderw')
    const url = computed(() => `https://dog.ceo/api/breed/${breed.value}/images/random` )
    const { x, y } = useMousePos();
    const { isPending, data, refresh, error } = useFetchData(url, 200);

    return { mouseX: x, mouseY: y, breed, isPending, data, refresh, error };
  },
  // data() {
  //   return {
  //     breed: 'collie/border',
